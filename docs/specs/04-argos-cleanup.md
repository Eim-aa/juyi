...
状态：**历史验收记录，已实现**（2026-07-13；P0 全部 + P1-1 取"删 § 引用改一句话注释"路径，P2 按规格仅记录）· 依赖：无。文中保留的是当时的明文 `volc.env` 语义；当前版本已迁移到 macOS 钥匙串，卸载规则以 [`scripts/uninstall.sh`](../../scripts/uninstall.sh) 为准。
